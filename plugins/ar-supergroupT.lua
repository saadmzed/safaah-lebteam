...
local function lock_group_links(msg, data, target)
  ...
  if group_link_lock == 'yes' then
    return 'رائـع حـــضـرت🚸الـــــمديـــــر/الادمـــــن تــــــم الـــــقــــــــفــل بـــــنــجـــــــاح🔐\nبواسطه 🚧📌 @'..msg.from.username..'\n'.."الرساله 🚧📌 "..msg.text.."\n"
  else
...
local function unlock_group_links(msg, data, target)
  ...
  if group_link_lock == 'no' then
    return 'رائـع حــــضـرت🚸الـــــمديـــــر/الادمـــــن تــــــم فـــــتـــــــــح بـــــنـــجـــــــاح🔐\nبواسطه🚧📌 @'..msg.from.username..'\n'.."الرساله 🚧📌"..msg.text.."\n" 
  else
...
local function set_rulesmod(msg, data, target)
  ...
  save_data(_config.moderation.data, data)
  return 'تم 🚧 وضع القوانين للمجموعه \n الرساله 💎 📌 '..msg.text..'\n'
end
...
local function promote_admin(receiver, member_username, user_id)
  ...
  if data[group]['moderators'][tostring(user_id)] then
    return send_large_msg(receiver, member_username..' رئيس عرفاء عباس ضمن👍🏿الادمنيه😂')
  end
...
local function promote2(receiver, member_username, user_id)
  ...
  local member_tag_username = string.gsub(member_username, '@', '(at)')
  if not data[group] then
    return send_large_msg(receiver, 'عزيزي المطور قم بتفعيلي🌚♥️ المجموعه غير فعالة')
  end
  if data[group]['moderators'][tostring(user_id)] then
  ...
  data[group]['moderators'][tostring(user_id)] = member_tag_username
  save_data(_config.moderation.data, data)
  send_large_msg(receiver, member_username..' تم رفعك🐸رئيس🔫عرفاء عباس ادمن')
end
...
local function demote2(receiver, member_username, user_id)
  ...
  if not data[group]['moderators'][tostring(user_id)] then
    return send_large_msg(receiver, member_username.. 'تم تنزيلك🐸من الرتبة🔫رئيس😂عرضاء')
  end
  data[group]['moderators'][tostring(user_id)] = nil
  save_data(_config.moderation.data, data)
  send_large_msg(receiver, member_username.. 'تم تنزيلك🐸من الرتبة🔫رئيس😂عرضاء')
end
...
function get_message_callback(extra, success, result)
    ...
    elseif get_cmd == "رفع المدير" then
        ...
        local channel_id = 'channel#id'..result.to.peer_id
            ...
            if result.from.username then
                text = " @"..result.from.username.."تم رفعك🐸مدير في🔫هذا المجموعه"
            else
                text = " @"..result.from.username.."تم رفعك🐸مدير في🔫هذا المجموعه"
            end
            send_large_msg(channel_id, text)
...
local function callbackres(extra, success, result)
  ...
  elseif get_cmd == "invite" then
    ...
    elseif get_cmd == 'mute_user' then
        ...
        if is_muted_user(chat_id, user_id) then
            unmute_user(chat_id, user_id)
            send_large_msg(receiver, " ["..user_id.."]تم الغاء كتمك🤐لاتشمر🐸بعد كلك😏بكد البعبوص")
        elseif is_momod(extra.msg) then
            mute_user(chat_id, user_id)
            send_large_msg(receiver, " ["..user_id.."]تم كتمك انزع🚧نعالك طب حافي😂ولاتوصخ👟الزولية")
        end
    end
end
    ...
    for k,v in pairs(result) do
        ...
        if vusername == member or vpeer_id == memberid then
            ...
            if group_owner then
                ...
                if result.username then
                    text = member_username.." @"..v.username.."| تم رفعك🐸فيترجي مضبوط😂بالمجموعه"
                else
                    text = " @"..v.username.."| تم رفعك🐸فيترجي مضبوط😂بالمجموعه "
                end
            end
        elseif memberid and vusername ~= member and vpeer_id ~= memberid then
            ...
            if group_owner then
                ...
                text = " ["..memberid.."] |  تم رفعك🐸فيترجي مضبوط😂بالمجموعه "
            end
        end
    end
 end
